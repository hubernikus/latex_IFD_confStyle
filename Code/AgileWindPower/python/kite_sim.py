...
# TODO: Libraries out of the function
...
def kite_sim(aircraft, params):
    ...
    #BRF
    qR = quat_mul(quat_inv(q),np.insert(R, 0, 0))
    qR_q = quat_mul(qR, q)
    R_b = qR_q[1:4]
    
    
    #Total external forces devided by glider's mass (linear acceleration)
    v_dot = (Faero_b + T_b  + R_b)/Mass + G_b  - np.cross(w,v)
    ...
    J = np.array([[Ixx, 0, Ixz], [0, Iyy, 00], [Ixz, 0, Izz]])
    
    #Angular motion equationin BRF
    Maero = np.array([L, M, N])
    #Moments rotation SRF -> BRF
    T_tmp = quat_mul(quat_inv(q_aoa), np.insert(Maero, 0, 0) )
    Trot = quat_mul(T_tmp, q_aoa)
    Maero = Trot[1:4]
    
    w_dot = np.inv(J) * (Maero - np.cross(w, J * w))
    
    #-----------------------------
    #Kinematic Equations: Position
    #-----------------------------
    # Aircraft position in the IRF
    qv = quat_mul(q, np.insert(v, 0, 0))
